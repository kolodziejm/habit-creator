...
class Dashboard extends Component {
  ...
  componentDidMount() {
    ...
    if (token.exp < Date.now() / 1000)
      return this.props.logoutUser(this.props.history, true);
    axios
      .get("/habits")
      .then(res => {
  ...
  openFinishSnackbar = (value, bonus, achievementCoins) => {
    const sentenceEnding =
      bonus > 0 && achievementCoins > 0
        ? ` + ${bonus} coins as a streak bonus and ${achievementCoins} for completing the achievement!`
        : bonus > 0
        ? ` + ${bonus} coins as a streak bonus!`
        : achievementCoins > 0
        ? ` + ${achievementCoins} for completing the achievement!`
        : ".";
    this.setState({
  ...
  render() {
    ...
    const finishedHabits = this.props.habits.habits.filter(
      habit => habit.isFinished
    ).length;
    const completedPercent = (
      (finishedHabits / this.props.habits.habits.length) *
      100
    ).toFixed(0);
        ...
        <main>
          ...
          {this.props.habits.loading ? (
            ...
            <div className={classes.noActiveHabits}>
              <Typography variant="h5" align="center">
                You currently don't have any active habits
              </Typography>
          ...
          >
            ...
            <DialogContent>
              <DialogContentText>
                Did you really finish that habit? Remember - cheating won't get
                you anywhere!
              </DialogContentText>
            </DialogContent>
            <DialogActions>
...
export default connect(
  mapStateToProps,
  { logoutUser, setHabits, finishHabit, updateCoins }
